...
class SessionGNN(nn.Module):
    """Session-based Graph Neural Network for user-job interactions"""
    ...
    def __init__(self, user_dim, job_dim, hidden_dim=64, num_layers=2):
        ...
        # Attention mechanism for session aggregation
    ...
    def forward(self, graph, user_features, job_features):
        ...
        # Combine user and job embeddings
        node_features = torch.cat([user_embeddings, job_embeddings], dim=0)
        ...
        # Get user embeddings from GNN output
        user_embeddings_gnn = h[:len(user_features)]
        
        # Session-level attention
        user_embeddings_gnn = user_embeddings_gnn.unsqueeze(0)  # (1, batch_size, hidden_dim)
        attended_features, _ = self.attention(
            user_embeddings_gnn, user_embeddings_gnn, user_embeddings_gnn
        )
...
class FairnessAwareJobRecommender(nn.Module):
    ...
    def forward(self, batch_data, graph=None):
        ...
        if graph is not None:
            ...
            # Fallback if no graph
            gnn_out = torch.zeros(wide_features.size(0), self.session_gnn.output_dim, 
                                device=wide_features.device)
...
def create_user_job_graph(interactions, user_ids, job_ids):
    """Create a bipartite graph from user-job interactions"""
    ...
    # Create DGL graph
    graph = dgl.graph((user_nodes, job_nodes), num_nodes=len(user_ids) + len(job_ids))
