...
class JobRecommendationTrainer:
    ...
    def prepare_data(self, df):
        ...
        print("Preprocessor feature keys:", self.preprocessor.feature_info.keys())
        print("DataFrame columns:", df_processed.columns.tolist())
        if 'user_gender' not in self.preprocessor.feature_info:
            self.preprocessor.feature_info['user_gender'] = 'categorical'
        ...
        # Try using create_user_job_graph from models; if it returns a PyG Data, use it.
        try:
            maybe_graph = create_user_job_graph(train_interactions, user_ids, job_ids)
        except Exception:
            maybe_graph = None

        if isinstance(maybe_graph, Data):
            # If the function already returned a PyG Data object, use it
            self.graph = maybe_graph.to(self.device)
            # If the returned Data contains helpful mappings, try to extract them
            if hasattr(maybe_graph, 'user_to_idx') and hasattr(maybe_graph, 'job_to_idx'):
                self.user_to_idx = maybe_graph.user_to_idx
                self.job_to_idx = maybe_graph.job_to_idx
            else:
                self.user_to_idx = {u: i for i, u in enumerate(user_ids)}
                self.job_to_idx = {j: i for i, j in enumerate(job_ids)}
        else:
            # Build PyG bipartite graph: users 0..U-1, jobs U..U+J-1
            # Build PyG bipartite graph: users 0..U-1, jobs U..U+J-1
            self.user_to_idx = {u: i for i, u in enumerate(user_ids)}
            self.job_to_idx = {j: i for i, j in enumerate(job_ids)}
            U = len(self.user_to_idx)
            J = len(self.job_to_idx)

            src, dst = [], []
            for _, row in train_interactions.iterrows():
                u = row['user_id']
                j = row['job_id']
                if (u in self.user_to_idx) and (j in self.job_to_idx):
                    src.append(self.user_to_idx[u])
                    dst.append(U + self.job_to_idx[j])  # offset job indices
            ...
            # ✅ Ensure no out-of-bound edge indices
            max_index = int(torch.max(edge_index)) if edge_index.numel() > 0 else 0
            num_nodes = max(U + J, max_index + 1)

            print(f"Graph stats: {U} users, {J} jobs, {len(src)} edges, {num_nodes} total nodes")
    ...
    def train_epoch(self, train_loader):
        ...
        for batch_idx, (X_batch, y_batch, sensitive_batch) in enumerate(tqdm(train_loader, desc="Training")):
            ...
            feature_names = list(self.preprocessor.feature_info.keys())

            # If there's one extra column, append a placeholder
            if num_features > len(feature_names):
                feature_names += [f"extra_feature_{i}" for i in range(num_features - len(feature_names))]

            batch_df = pd.DataFrame(X_batch.numpy(), columns=feature_names)
